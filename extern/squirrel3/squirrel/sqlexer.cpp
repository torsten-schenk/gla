...
/* TODO GLA sqcompiler.cpp around line 1479: SQFunctate->AddLineInfos requires an additional string parameter to update sourcename */
void SQLexer::LexPreProcess()
{
	INIT_TEMP_STRING();
	for(;;) {
		NEXT();
		if(CUR_CHAR == _SC('\n') || IS_EOB())
			break;
		else if(CUR_CHAR != _SC('\t') && CUR_CHAR != _SC(' ') && CUR_CHAR != _SC('\r'))
			APPEND_CHAR(CUR_CHAR);
	}
	TERMINATE_BUFFER();
	int colonidx = -1;
	if(_longstr[0] != _SC('!')) {
		Error(_SC("unsupported processor directive"));
		return;
	}
	for(int i = 0; i < _longstr.size(); i++) {
		if(_longstr[i] == _SC(':')) {
			if(colonidx < 0)
				...
				Error(_SC("invalid preprocessor directive: multiple colons found for instruction 'set location'"));
				return;
			}
		}
	}
	if(colonidx == -1) {
		Error(_SC("invalid preprocessor directive: no colon found for instruction 'set location'"));
		return;
	}
	_longstr[colonidx] = _SC('\0');
	SQUnsignedInteger line;
	LexInteger(&_longstr[colonidx + 1], &line);
	_currentline = line - 1;
}
...
/*		case _SC('$'):
			if(_currentcolumn != 1) {
				Error(_SC("preprocessor instructions only allowed at beginning of line"));
				LexLineComment();
				continue;
			}
			LexPreProcess();
